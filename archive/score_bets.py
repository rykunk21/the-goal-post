...
def evaluate_bet(row: dict, result: dict) -> str:
    ...
    if market == "Spread":
        # Side is like "XAVI +13.5" or "OKST -3.5"
        tokens = side.split()
        if len(tokens) < 2:
            return "?"
        try:
            ...
            return "?"
        # Figure out if this is the away or home team spread
        side_tok = tokens[0]
        is_away = (side_tok == result["away_abbr"].upper() or
                   any(side_tok in t.upper() for t in result["away_team"].split()))
